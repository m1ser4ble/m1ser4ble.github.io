...
categories: backend
excerpt: "POJO = Plain Old Java Object = 평범한 옛날 자바 객체 DTO = Data Transfer Object = 데이터 전송 객체 SSE = Server-Sent Events..."
toc: true
toc_sticky: true
tags: [backend, guide]
source: "/home/dwkim/dwkim/docs/backend/backend-기본개념.md"
---
TL;DR
- POJO = Plain Old Java Object = 평범한 옛날 자바 객체 DTO = Data Transfer Object = 데이터 전송 객체 SSE = Server-Sent Events = 서버가 보내는 이벤트 특성 SSE WebSocket 방향 단방향 (서버→클라이언트) 양방향 프로토콜 HTTP WS 재연결 자동 수동 구현 브라우저 지원 좋음 좋음 용도 알림, 피드 채팅, 게임 JWT = JSON Web Token = JSON 형식의 웹 토큰 CORS = Cross-Origin Resource Sharing = 교차 출처 리소스 공유 POJO, DTO, SSE, JPA, ORM, Hibernate, Hexagonal Architecture, Ports and Adapters, DI, DIP, SOLID, JWT, JSON Web Token, CORS, SOP, Cross-Origin
