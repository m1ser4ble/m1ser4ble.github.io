...
categories: backend
excerpt: "Asynchronous Request-Reply와 Server-Side Orchestration은 장시간 작업을 접수와 처리로 분리하고 백엔드가 워크플로를 소유하게 해 타임아웃과 결합도를 줄이는 비동기 설계 패턴이다."
toc: true
toc_sticky: true
tags: [backend, async, orchestration, polling, saga, workflow]
source: "/home/dwkim/dwkim/docs/backend/async-request-reply-서버사이드-오케스트레이션.md"
---
TL;DR
- 긴 처리 시간을 가진 작업은 동기 HTTP 한 번으로 끝내려 하지 말고 접수와 완료를 분리하는 비동기 패턴으로 설계하는 편이 안전하다.
- Request-Reply, polling, webhook, saga, workflow engine은 모두 같은 문제를 서로 다른 수준에서 푸는 도구들이다.
- 원문 전체는 아래 상세 내용에 그대로 포함했다.

## 1. 개념
Asynchronous Request-Reply는 요청 접수와 실제 완료 시점을 분리해 장시간 작업을 안전하게 처리하는 패턴이고, Server-Side Orchestration은 그 과정의 호출 순서·상태 전이·실패 복구를 프론트엔드가 아니라 백엔드가 중앙에서 통제하는 방식이다.

## 2. 배경
동기 HTTP는 로드밸런서 타임아웃, 모바일 연결 단절, 긴 서비스 체인, 스레드 점유 같은 구조적 한계를 가진다. 특히 리포트 생성, 인코딩, ML 추론, 다중 서비스 조합처럼 수 초에서 수 분이 걸리는 작업은 클라이언트 연결을 오래 붙잡을수록 UX와 안정성이 함께 나빠진다.

## 3. 이유
비동기 Request-Reply로 바꾸면 서버는 빠르게 202 Accepted와 상태 조회 지점을 반환하고, 이후 처리는 큐·워크플로·오케스트레이터가 맡을 수 있다. 이렇게 하면 타임아웃 위험을 줄이고, 프론트엔드가 내부 서비스 토폴로지를 알 필요도 없어지며, 재시도·보상 트랜잭션·멱등성 같은 운영 제어를 백엔드에 집중시킬 수 있다.

## 4. 특징
- **접수와 실행 분리**: 202 Accepted, Location, Retry-After 같은 HTTP 의미론을 활용한다.
- **백엔드 중심 제어**: BFF나 오케스트레이터가 서비스 호출 순서와 데이터 조합을 책임진다.
- **복구 가능성 강화**: saga, workflow engine, durable execution으로 실패 복구와 재시도를 체계화할 수 있다.
- **클라이언트 단순화**: 프론트엔드는 상태 확인·완료 수신에 집중하고 내부 의존성을 덜 안다.
- **확장 가능한 패턴 스펙트럼**: 단순 polling부터 Temporal·Step Functions 같은 워크플로 엔진까지 같은 축에서 비교할 수 있다.
