...
categories: frontend
excerpt: "Mold 컴포넌트 라이브러리 (CCK 자체 디자인 시스템)은(는) 핵심 개념과 배경, 이유를 정리해 적용 기준을 제공한다."
toc: true
toc_sticky: true
tags: [Mold, CCK, 래퍼, Wrapper, Carbon]
---

## TL;DR
- Mold 컴포넌트 라이브러리 (CCK 자체 디자인 시스템)의 핵심 개념과 용어를 한눈에 정리한다.
- Mold 컴포넌트 라이브러리 (CCK 자체 디자인 시스템)이(가) 등장한 배경과 필요성을 요약한다.
- Mold 컴포넌트 라이브러리 (CCK 자체 디자인 시스템)의 특징과 적용 포인트를 빠르게 확인한다.

## 1. 개념
Mold 컴포넌트 라이브러리 (CCK 자체 디자인 시스템)은(는) 핵심 용어와 정의를 정리한 주제로, 개발/운영 맥락에서 무엇을 의미하는지 설명한다.
