---
layout: single
title: "Mold 컴포넌트 라이브러리 (사내 디자인 시스템)"
date: 2026-02-14 09:01:36 +0900
categories: frontend
excerpt: "Mold 컴포넌트 라이브러리 (사내 디자인 시스템)은(는) 핵심 개념과 배경, 이유를 정리해 적용 기준을 제공한다."
toc: true
toc_sticky: true
tags: [mold, internal, wrapper, carbon]
---

## TL;DR
- Mold 컴포넌트 라이브러리 (사내 디자인 시스템)의 핵심 개념과 용어를 한눈에 정리한다.
- Mold 컴포넌트 라이브러리 (사내 디자인 시스템)이(가) 등장한 배경과 필요성을 요약한다.
- Mold 컴포넌트 라이브러리 (사내 디자인 시스템)의 특징과 적용 포인트를 빠르게 확인한다.

## 1. 개념
Mold 컴포넌트 라이브러리 (사내 디자인 시스템)은(는) 핵심 용어와 정의를 정리한 주제로, 개발/운영 맥락에서 무엇을 의미하는지 설명한다.
...
## 5. 상세 내용
> **작성일**: 2026-01-30
> **카테고리**: Frontend / Design System / 사내
> **포함 내용**: Mold, 래퍼 컴포넌트, Carbon, Ant Design, 사내 타이포그래피, 디자이너-개발자 협업
...
정체: 사내 프로젝트 전용 컴포넌트 래퍼 라이브러리
위치: /common/mold/
역할: Carbon + Ant Design을 사내 브랜드로 "틀"에 맞춰 찍어냄
```
...
│  │   Carbon Button                                              │ │
│  │        ↓                                                     │ │
│  │   Mold에서 래핑 (사내 타이포그래피 적용)                     │ │
│  │        ↓                                                     │ │
│  │   MoldButton                                                 │ │
│  │        ↓                                                     │ │
│  │   app-a, app-b, app-c 모든 앱에서 import                │ │
│  │                                                              │ │
│  └────────────────────────────────────────────────────────────┘ │
│                                                                   │
│  장점:                                                            │
...
│  │                        앱 레이어                             ││
│  │  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐        ││
│  │  │  app-a  │  │app-b│ │  app-c   │  │ core-service│       ││
│  │  └────┬────┘  └────┬────┘  └────┬────┘  └────┬────┘        ││
│  │       │            │            │            │              ││
│  │       └────────────┴─────┬──────┴────────────┘              ││
│  │                          │                                   ││
...
│  │    orgVariant?: 'primary' | 'secondary';  // 이미 Carbon에 있음!  │
│  │  }                                                           │ │
│  │                                                              │ │
│  │  // ✅ 좋은 예 - 필요한 것만 추가                            │ │
│  │  interface MoldButtonProps extends ButtonProps {             │ │
...
│  │  import { LocaleProvider } from '@org/mold';                 │ │
│  │                                                              │ │
...
```tsx
import { MoldDataTable } from '@org/mold';
...
{
  "dependencies": {
    "@org/mold": "workspace:~1.0.0",
    "@org/typography": "workspace:~1.0.0"
  }
}
```
...
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  Mold = 사내 프로젝트 전용 컴포넌트 래퍼 라이브러리                 │
│                                                                   │
│  등장 배경:                                                        │
│  ├── Carbon/Ant Design 폰트가 사내 브랜드와 안 맞음                │
│  ├── 개발자마다 다른 컴포넌트 선택                                │
│  ├── 반복적인 커스터마이징 작업                                   │
│  └── 디자이너-개발자 소통 비용                                    │
│                                                                   │
│  핵심 원칙:                                                        │
│  ├── 타이포그래피만 사내로 교체 (색상/배경/테두리 유지)            │
│  ├── Carbon = 데이터 중심 컴포넌트                                │
...
│                                                                   │
│  사용 방법:                                                        │
│  ├── import { MoldButton } from '@org/mold';                      │
│  ├── Carbon/Ant와 동일한 API                                      │
│  └── 사내 타이포그래피 자동 적용                                   │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
...
`Mold`, `사내`, `래퍼`, `Wrapper`, `Carbon`, `Ant Design`, `디자인 시스템`, `타이포그래피`, `컴포넌트 라이브러리`, `Figma`, `Storybook`, `디자이너-개발자 협업`
