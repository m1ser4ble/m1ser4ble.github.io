...
# TL;DR
- **Claude Code OOM (Out of Memory) 대응의 핵심 개념과 사용 범위를 한눈에 정리**
...
## 1. 개념
``` ┌─────────────────────────────────────────────────────────────┐ │                                                             │ │  증상:                                                      │ │  ├── Claude Code 사용 중 SSH 연결 끊김                     │ │  ├── 재접속하면 tmux 세션도 없음                           │ │  ├── 특히 여러 subagent 동시 실행 시 빈번                  │ │  └── Docker 컨테이너도 함께 죽기도 함                      │ │                                                             │ └─────────────────────────────────────────────────────────────┘ ```

## 2. 배경
Claude Code OOM (Out of Memory) 대응이(가) 등장한 배경과 기존 한계를 정리한다.
