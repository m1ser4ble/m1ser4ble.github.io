...
categories: security
excerpt: "1. [개요: 세 패턴의 전체 그림](#1-개요-세-패턴의-전체-그림)"
toc: true
toc_sticky: true
tags: [security, bff, same, origin, reverse]
source: "/home/dwkim/dwkim/docs/security/bff-리버스프록시-토큰릴레이-완전가이드.md"
---
TL;DR
- BFF, Same-Origin Reverse Proxy, Token Relay 완전 가이드의 핵심 개념을 빠르게 파악할 수 있다.
- 배경과 이유를 통해 왜 필요한지 맥락을 이해할 수 있다.
- 특징과 상세 내용을 통해 실무 적용 포인트를 확인할 수 있다.

## 1. 개념
BFF, Same-Origin Reverse Proxy, Token Relay 완전 가이드의 핵심 정의와 문제 공간을 간단히 정리한다.
...
| 상황 | 추천 패턴 | 이유 |
|------|-----------|------|
| **소규모 SPA + 단일 API** | Same-Origin Reverse Proxy만 | CORS 제거로 충분. BFF 오버엔지니어링. |
| **SPA + 다중 마이크로서비스** | Reverse Proxy + BFF | API 집계와 보안 경계 필요. |
| **SPA + OAuth/OIDC 인증** | Reverse Proxy + BFF + Token Relay | 토큰을 서버에 보관하는 완전한 보안. |
| **Next.js/Nuxt 풀스택** | 프레임워크 내장 BFF + Token Relay | 별도 BFF 서버 불필요. 프레임워크가 모두 제공. |
| **모바일 + 웹 동시 지원** | API Gateway + 클라이언트별 BFF | 각 클라이언트에 최적화된 API. |
| **높은 보안 요구 (금융)** | Reverse Proxy + BFF + Token Relay + mTLS | 모든 계층에서 보안 강화. |
| **레거시 모놀리스** | Same-Origin (이미 해당) | 프론트/백 분리가 없으므로 추가 패턴 불필요. |
