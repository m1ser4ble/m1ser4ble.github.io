...
categories: backend
excerpt: "사내 프로젝트에서는 모든 API 엔드포인트에 **POST 메서드만 사용**하는 정책을 적용하고 있습니다. 이 문서에서는 이러한 설계 결정의 배경, 보안적 이점, 기술적 장점을 상세히 설명합니다."
toc: true
toc_sticky: true
tags: [backend, post, only, api, 설계]
source: "/home/dwkim/dwkim/docs/backend/post-only-api-설계.md"
---
TL;DR
- POST only API 설계 패턴의 핵심 개념을 빠르게 파악할 수 있다.
- 배경과 이유를 통해 왜 필요한지 맥락을 이해할 수 있다.
- 특징과 상세 내용을 통해 실무 적용 포인트를 확인할 수 있다.

## 1. 개념
POST only API 설계 패턴의 핵심 정의와 문제 공간을 간단히 정리한다.
...
> **작성일**: 2026-02-02
> **키워드**: POST only, API 설계, 보안, CSRF, URL 노출, RPC, RESTful, 사내 가이드라인

---

## 개요

사내 프로젝트에서는 모든 API 엔드포인트에 **POST 메서드만 사용**하는 정책을 적용하고 있습니다. 이 문서에서는 이러한 설계 결정의 배경, 보안적 이점, 기술적 장점을 상세히 설명합니다.
...
`POST only`, `API 설계`, `RPC`, `RESTful`, `CSRF`, `URL 노출`, `보안`, `캐싱`, `PathVariable`, `RequestBody`, `kebab-case`, `사내 가이드라인`, `일관성`, `확장성`
