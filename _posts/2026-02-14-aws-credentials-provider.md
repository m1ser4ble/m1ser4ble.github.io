...
## TL;DR
- AWS Credentials Provider의 핵심 개념과 용어를 한눈에 정리한다.
- AWS Credentials Provider이(가) 등장한 배경과 필요성을 요약한다.
- AWS Credentials Provider의 특징과 적용 포인트를 빠르게 확인한다.

## 1. 개념
AWS Credentials Provider은(는) 핵심 용어와 정의를 정리한 주제로, 개발/운영 맥락에서 무엇을 의미하는지 설명한다.
