...
categories: backend
excerpt: "Streamlit, Python, 데이터 앱, 대시보드, Session State, 캐싱, WebSocket, Gradio, Dash"
toc: true
toc_sticky: true
tags: [backend, streamlit, server]
source: "/home/dwkim/dwkim/docs/backend/streamlit-서버.md"
---
TL;DR
- Streamlit, Python, 데이터 앱, 대시보드, Session State, 캐싱, WebSocket, Gradio, Dash
- Streamlit Server를 알아두면 설계 판단과 구현 선택을 더 분명하게 할 수 있다.
...
## 2. 배경
Streamlit Server가 등장한 배경과 문제 상황을 이해하는 데 도움이 된다.

## 3. 이유
Streamlit Server를 알아두면 설계 판단과 구현 선택을 더 분명하게 할 수 있다.

## 4. 특징
Streamlit Server의 특징, 장단점, 적용 포인트를 원문에서 자세히 확인할 수 있다.
