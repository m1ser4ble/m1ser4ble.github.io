...
categories: backend
excerpt: "k6와 hey는 부하 테스트를 코드화하거나 빠른 벤치마크로 성능 병목을 드러낸다"
toc: true
toc_sticky: true
tags: [k6, hey, loadtesting, benchmark, performance]
---

**TL;DR**
- k6는 JavaScript로 시나리오를 코드화해 현실적인 부하 테스트를 만든다.
- hey는 단일 엔드포인트의 빠른 HTTP 벤치마크에 최적화됐다.
- 목적(시나리오 vs 즉석 측정)에 따라 도구를 선택하는 것이 핵심이다.

## 1. 개념
k6와 hey는 모두 HTTP 부하 테스트 도구지만 목적이 다르다. k6는 “Testing as Code” 철학으로 사용자 시나리오를 코드로 정의하고, hey는 간단한 CLI로 빠른 벤치마크를 수행한다. 즉, **복잡한 시나리오 검증**이 필요하면 k6, **간단한 성능 측정**이 필요하면 hey가 적합하다.

## 2. 배경
전통적 부하 테스트 도구(JMeter, ab 등)는 성능 오버헤드나 복잡한 구성 문제가 있었다. k6는 Go 엔진과 JS 스크립팅으로 개발자 친화적인 접근을 제공했고, hey는 ab의 한계를 Go 동시성으로 해결해 간편함을 극대화했다.

## 3. 이유
현대 서비스는 CI/CD, 관측성, 자동화 요구가 커졌다. k6는 시나리오 기반 테스트와 Grafana 생태계를 통해 이러한 요구를 충족하고, hey는 릴리스 직후 빠른 성능 확인 같은 상황에 실용적인 선택이 된다.

## 4. 특징
- k6: Go 엔진 + JS 스크립팅, 시나리오/Threshold 기반 자동 판정, Grafana 연계.
- hey: 단일 바이너리, CLI 한 줄 실행, HTTP/2 지원, 퍼센타일 분포 제공.
...
- **Game Days**: 전사적으로 참여하는 장애 시뮬레이션 행사. 부하 테스트 + 인프라 장애 + 의사결정 훈련을 통합 검증
