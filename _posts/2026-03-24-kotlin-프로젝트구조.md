...
categories: backend
excerpt: "장점: - 단순하고 익숙함 (Spring Boot 튜토리얼 기본 구조) - 계층이 명확하게 보임 - 소규모 프로젝트에서 빠르게 시작 가능 단점: - 주문 기능 하나 수정 시 4~5개 패키지를..."
toc: true
toc_sticky: true
tags: [backend, kotlin]
source: "/home/dwkim/dwkim/docs/backend/kotlin-프로젝트구조.md"
---
TL;DR
- 장점: - 단순하고 익숙함 (Spring Boot 튜토리얼 기본 구조) - 계층이 명확하게 보임 - 소규모 프로젝트에서 빠르게 시작 가능 단점: - "주문" 기능 하나 수정 시 4~5개 패키지를 오가야 함 - 관련 없는 파일들이 같은 폴더에 섞임 - 코드 구조만 보고 이 앱이 뭘 하는지 파악 불가 장점: - 주문 관련 코드가 한 곳에 모임 - 기능 추가/삭제 = 패키지 추가/삭제 - 코드 구조가 앱이 하는 일을 보여줌 - Kotlin의 internal 가시성으로 패키지 내 캡슐화 가능 - 마이크로서비스 추출 용이 Feature + Layer를 결합한 구조: Kotlin, 프로젝트 구조, Package by Layer, Package by Feature, Screaming Architecture, DDD, Bounded Context, 멀티 모듈, Gradle, internal, sealed class, data class, Convention over Configuration, 의존성 역전, DIP, Clean Architecture, Layered Architecture, Aggregate Root, Value Object, Use Case
- - 계층이 명확하게 보임
