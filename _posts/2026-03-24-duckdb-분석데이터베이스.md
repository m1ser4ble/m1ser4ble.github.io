...
categories: backend
excerpt: "SQLite DuckDB PostgreSQL Spark 용도 OLTP OLAP OLTP+OLAP 대규모 OLAP 설치 없음 없음 서버 필요 클러스터 필요 저장 방식 행 지향 열 지향 행 지향..."
toc: true
toc_sticky: true
tags: [backend, duckdb]
...
---
TL;DR
- SQLite DuckDB PostgreSQL Spark 용도 OLTP OLAP OLTP+OLAP 대규모 OLAP 설치 없음 없음 서버 필요 클러스터 필요 저장 방식 행 지향 열 지향 행 지향 열 지향 분석 속도 느림 빠름 보통 빠름 데이터 크기 ~수 GB ~수백 GB ~수 TB ~수 PB 동시 접속 제한적 단일 프로세스 수백~수천 수백 트랜잭션 지원 제한적 완전 지원 없음 적합한 사용 앱 내장 DB 로컬 분석 서비스 DB 빅데이터 DuckDB, OLAP, OLTP, Columnar, 열 지향, 행 지향, 임베디드 DB, in-process, pandas, Parquet, SQLite, BigQuery, Snowflake, Spark, 분석 데이터베이스, 벡터화 실행, CWI
